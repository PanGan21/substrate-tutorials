...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn mint(
			...
			let id = Self::nonce();
			let unique_asset_details = UniqueAssetDetails::new(origin.clone(), metadata, supply);
			UniqueAsset::<T>::insert(id, unique_asset_details);
		...
		pub fn burn(origin: OriginFor<T>, asset_id: UniqueAssetId, amount: u128) -> DispatchResult {
			let origin = ensure_signed(origin)?;
				...
				UniqueAsset::<T>::get(asset_id).is_some(),
			...
			UniqueAsset::<T>::try_mutate(asset_id, |maybe_details| -> DispatchResult {
				...
				Account::<T>::mutate(asset_id, origin.clone(), |balance| {
					let old_supply = *balance;
					*balance = old_supply.saturating_sub(amount);
					burned_amount = old_supply - *balance;
				});
...
impl<T: Config> Pallet<T> {
	fn ensure_own_some(asset_id: UniqueAssetId, account: T::AccountId) -> Result<(), Error<T>> {
		let balance = Self::account(asset_id, account);

		ensure!(balance > 0, Error::<T>::NotOwned);
